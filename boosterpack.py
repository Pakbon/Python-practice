...
day = day.replace(hour=0,minute=1,second=0,microsecond=0)\
                  - datetime.timedelta(1) #tracking YESTERDAY's haul
...
def bpacks():
    ...
    with open('id.txt') as input:
        ...
        url = 'https://steamcommunity.com/inventory/{}/753/6?count=10'\
               .format(j['steam64'])
    ...
    with open('id.txt') as input:
        j = json.loads(input.read())
        url = 'http://127.0.0.1:1242/Api/Command/unpack%20{}'\
               .format(j['bot'])
